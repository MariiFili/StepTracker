public class StepTracker {
 int AbitionStep = 10000;
    ...
    void saveCountStep(int month, int day, int step) {
        monthToData[month].daysDate[day-1] = step;
    }
    void statistic(int mohth) {
        ...
        System.out.println("Пройденная дистанция - " + converter.distance(sum));
        System.out.println("Сожженые калории - " + converter.kalor(sum));
      }

    void AmbStep(int ambition) {
        System.out.println("Новая цель по количеству шагов " + ambition);
        AbitionStep = ambition;
    }
    ...
    void maxSeries(int month) {
            ...
            for (int i= 0; i < monthToData.length; i++) {
                if (monthToData[month].daysDate[i] >= AbitionStep) {
                    index++;
                }
                else index = 0;
                if (index > max) {
